...
from mobsf.mobsf_client import upload_and_scan, get_static_json, run_dynamic_analysis
...
async def scan_apk(file: UploadFile = File(...), dynamic: bool = False):
    ...
    static_json, dynamic_json = None, None
    ...
    try:
        if USE_MOBSF:
            ...
            dynamic_json = run_dynamic_analysis(scan_hash) if dynamic else None
    ...
    report_path = generate_report(
        ...
        features=feats,
        mobsf_excerpt=None
        if analysis_backend == "Androguard"
        else {
